...
if __name__ == '__main__':
    ...
    #dim_list = [1,2,4,8]
    dim_list = [3]
    m_intra = 1#np.array(int(sys.argv[1]))
    N_simulations = 10**6
    M_repetitions = 50
    #threshold_quantiles = np.linspace(10, 0.01, 100)
    threshold_quantiles = [0.65]
    threshold_single = [0.65]
    ...
    for dim in dim_list:
        ...
        instance_compare_samplers_mc.simulate_and_extract(threshold_quantiles, threshold_single, target_function_mean, [theta_sampler_mc], ['MC'])
        #instance_compare_samplers_mc.simulate_and_extract(threshold_quantiles, threshold_single, target_function_var, [theta_sampler_mc], ['MC'])
        import ipdb; ipdb.set_trace()
        name_plot = "mean_mixed_gaussian_static_dim_%s_m_%s_revision.png" % (dim, m_intra)
        print 'now plotting'
    ...
    if False: 
        ...
        pickle.dump(vars_to_save, open('backup_vars_revision.p', "wb"))
        pickle.dump(means_to_save, open('backup_means_revision.p', "wb"))
    pdb.set_trace()
