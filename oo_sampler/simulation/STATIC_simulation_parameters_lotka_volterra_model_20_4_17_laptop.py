...
epsilon_target = functions_mixture_model.epsilon_target(dim_particles) #0.001 #0.25
...
filename = functions_mixture_model.model_string+'_negative_binomial_uniform_kernel_1_VB_component_fixed_epsilon_schedule_algo_only_dim3'
...
for sampler in sampler_list:
    ...
    array_results = np.zeros((2, dim_particles, length_quantiles, repetitions)) 
    for k_repetion in range(repetitions):
        test_sampler.accept_reject_sampler(100000)
        for j_quantile in range(length_quantiles):
            #pdb.set_trace()
            posterior = test_sampler.f_accept_reject_precalculated_particles(test_sampler.particles_AR_initial, test_sampler.auxialiary_particles_accept_reject.flatten(), percentile=quantiles[j_quantile])
...
plt.hist(posterior_list[0][0,:]); plt.show()
...
posterior_qmc = test_sampler.f_accept_reject_precalculated_particles(test_sampler.particles_AR_initial, test_sampler.auxialiary_particles_accept_reject.flatten(), epsilon_target_accept_reject=0.01)

test_sampler.setInitiationFunction(functions_mixture_model.theta_sampler_mc)
test_sampler.accept_reject_sampler(1000000)
posterior_mc = test_sampler.f_accept_reject_precalculated_particles(test_sampler.particles_AR_initial, test_sampler.auxialiary_particles_accept_reject.flatten(), epsilon_target_accept_reject=0.01)
